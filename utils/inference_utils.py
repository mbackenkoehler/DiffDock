...
def ligand_to_pocket_center(data_list, pocket):
    if pocket is None:
        return

    for complex_graph in data_list:
        pocket_center = complex_graph["receptor"].pos[pocket].mean(0)
        molecule_center = torch.mean(complex_graph["ligand"].pos, dim=0, keepdim=True)
        complex_graph["ligand"].pos += pocket_center - molecule_center
